...
const ServiceDetail = () => {
  ...
  const { data: procedures = [] } = useQuery({
    queryKey: ["service-procedures", id],
    queryFn: async () => {
      const { data, error } = await supabase
    ...
    <div className="pb-24">
      ...
      <div className="relative bg-gradient-to-br from-primary/90 to-primary px-5 pb-10 pt-4">
        <div className="flex items-center justify-between">
          <button
            onClick={() => navigate(-1)}
            className="flex h-8 w-8 items-center justify-center rounded-full bg-background/20 text-primary-foreground"
          >
            <ArrowLeft size={18} />
          </button>
      ...
      {procedures.length > 0 && (
        <ProcedureSteps
          steps={procedures.map((proc, index) => ({
            id: proc.step_number,
            title: proc.title,
        ...
        <div className="mx-auto max-w-lg">
          <Button
            ...
            onClick={() =>
              navigate(
                isAuthenticated
                  ? `/appointment/book?service=${encodeURIComponent(service.name)}`
                  : "/signin",
              )
            }
          >
          ...
          {service.next_slot && (
            <p className="mt-1 text-center text-[10px] text-muted-foreground">
              Next available slot: {service.next_slot}
            </p>
          )}
        </div>
      </div>
