  ...
  return (
    <div className="px-5 pt-6 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { label: "Total", value: counts.all, color: "bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400" },
          { label: "Pending", value: counts.pending, color: "bg-amber-50 text-amber-600 dark:bg-amber-950 dark:text-amber-400" },
          { label: "Completed", value: counts.completed, color: "bg-emerald-50 text-emerald-600 dark:bg-emerald-950 dark:text-emerald-400" },
        ].map((s) => (
      ...
      <div className="relative">
        ...
        <Input placeholder="Search patients, services..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-9 text-sm" />
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="w-full grid grid-cols-4 h-9">
          ...
          <TabsTrigger value="completed" className="text-[11px]">Done</TabsTrigger>
        </TabsList>
      </Tabs>
      ...
      <div className="space-y-2">
        ...
        {filtered.map((appt) => (
          <Card key={appt.id} className="border shadow-sm cursor-pointer transition-colors hover:bg-muted/50" onClick={() => setSelectedAppt(appt)}>
            <CardContent className="p-3.5">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-foreground truncate">{appt.patient_name || "Self"}</p>
                  <p className="text-[11px] text-muted-foreground">{appt.services?.name}</p>
                  <p className="text-[11px] text-muted-foreground">{appt.doctors?.name}</p>
                </div>
                <div className="text-right shrink-0 ml-2">
                  <Badge variant="secondary" className={`text-[9px] px-1.5 py-0.5 ${statusColors[appt.status]}`}>{appt.status}</Badge>
                  ...
                  <p className="text-[10px] text-muted-foreground">{appt.appointment_date}</p>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <Dialog open={!!selectedAppt} onOpenChange={() => setSelectedAppt(null)}>
        <DialogContent className="max-w-sm mx-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-muted-foreground">Review and manage this appointment</DialogDescription>
          </DialogHeader>
          {selectedAppt && (
            <div className="space-y-3">
              <div className="space-y-2">
                {[
                  { label: "Patient", value: selectedAppt.patient_name || "Self" },
                  { label: "Service", value: selectedAppt.services?.name || "" },
                  { label: "Doctor", value: selectedAppt.doctors?.name || "" },
                  { label: "Date", value: selectedAppt.appointment_date },
                ...
                <div className="flex justify-between text-sm items-center">
                  ...
                  <Badge variant="secondary" className={`text-[10px] ${statusColors[selectedAppt.status]}`}>{selectedAppt.status}</Badge>
                </div>
              </div>
              {selectedAppt.status === "Pending" && (
                <div className="grid grid-cols-2 gap-2 pt-2">
                  <Button size="sm" className="text-xs gap-1" onClick={() => updateStatus(selectedAppt.id, "Approved")}>
